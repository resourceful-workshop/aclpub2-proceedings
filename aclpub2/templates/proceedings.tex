...
\definecolor{falured}{rgb}{0.48, 0.1, 0.1}
...
\begin{titlepage}
  ...
  \backgroundsetup{
            contents={\includegraphics{/Users/xilini/aclpub2_proceedings/resourceful-2023/prefaces/faroe.jpg}}
        }
  
  \begin{center}
    \vspace{1.5cm}

    {\LARGE \textbf{\textcolor{falured}{\VAR{conference.anthology_venue_id} \VAR{conference.start_date.year}}}}

    \vspace*{65mm}

    \vspace*{3cm}
    {\bf\huge \transparent{2}\textcolor{falured}{\VAR{conference.event_name}}}

    \vspace*{1cm}

    {\bf\LARGE \textcolor{falured}{\VAR{conference.cover_subtitle}}}
    ...
    \vspace*{1cm}
    {\LARGE \textcolor{white}{Editors: Nikolai Ilinykh, Felix Morger, Dana Dannélls, Simon Dobnik, Beáta Megyesi, Joakim Nivre}}
    \vspace{-1cm}
...
\BLOCK{macro generate_sponsor_tier(tier)}
  ...
  \BLOCK{for logo_batched in tier.logos|batch(4)}
    ...
    \BLOCK{if logo is equalto('sbx-logo.pdf')}
    \begin{minipage}[c][0.21\linewidth][c]{0.21\linewidth}
      \includegraphics[width=2\linewidth]{\VAR{root}/sponsor_logos/\VAR{logo}}
    \end{minipage}\hspace{0.2\linewidth}
    \BLOCK{elif logo is equalto('CLASP-orange.pdf')}
    \begin{minipage}[c][0.21\linewidth][c]{0.21\linewidth}
    \includegraphics[width=2\linewidth]{\VAR{root}/sponsor_logos/\VAR{logo}}
    \end{minipage}\hspace{0.2\linewidth}
    \BLOCK{else}
